...
- Firmware version: `t-dev-089-041-015`
- Firmware origin/fork: `tumoflip`
- Firmware API: `88.0`
- Target: Flipper Zero F7
- Release channel: `dev experimental line`
- Release package: `flipper-z-f7-update-t-dev-089-041-015.tgz`
...
- `015`: development iteration inside the tumoflip internal build version.
...
firmware prefix. The current development line reports `t-dev-089-041-015`
through `device_info`.
...
| --- | --- | --- |
| Firmware identity | Reports itself as Unleashed. | Reports `firmware_version: t-dev-089-041-015` and `firmware_origin_fork: tumoflip`. |
...
| NFC additions | Uses the Unleashed 089 NFC feature set. | Shows captured MIFARE Ultralight/NTAG PWD and PACK, adds the Bambu Lab filament spool parser, and supports large ISO15693 multi-block emulation with bounded parser writes. |
...
| Build metadata | Uses upstream build metadata conventions. | Uses `t-dev-089-041-015` for the installed firmware version and release artifact suffix, while keeping `tumoflip` as the fork origin. |
...
- `flipper-z-f7-update-t-dev-089-041-015.tgz`
...
```text
dist/f7-C/flipper-z-f7-update-t-dev-089-041-015.tgz
```

...
- Firmware version: `tmwhflpprarf089-035`
- Firmware origin/fork: `tumoflip`
- Firmware API: `87.16`
- Target: Flipper Zero F7
- Release channel: `main` stable line
- Release package: `flipper-z-f7-update-tmwhflpprarf089-035.tgz`
...
| --- | --- | --- |
| Firmware identity | Reports itself as Unleashed. | Reports `firmware_version: tmwhflpprarf089-035` and `firmware_origin_fork: tumoflip`. |
...
| Build metadata | Uses upstream build metadata conventions. | Uses `tmwhflpprarf089-035` for the installed firmware version and release artifact suffix, while keeping `tumoflip` as the fork origin. |
...
Stable `main` update packages are published on
[GitHub Releases](https://github.com/squazaryu/tumoflip/releases). The current
stable build uses this artifact name:

- `flipper-z-f7-update-tmwhflpprarf089-035.tgz`
...
```text
dist/f7-C/flipper-z-f7-update-tmwhflpprarf089-035.tgz
```

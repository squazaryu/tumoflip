...
- Firmware version: `t-dev-089-041-011`
- Firmware origin/fork: `tumoflip`
- Firmware API: `88.0`
- Target: Flipper Zero F7
- Release channel: `dev experimental line`
- Release package: `flipper-z-f7-update-t-dev-089-041-011.tgz`
...
firmware prefix. The current development line reports `t-dev-089-041-011`
through `device_info`.
...
| --- | --- | --- |
| Firmware identity | Reports itself as Unleashed. | Reports `firmware_version: t-dev-089-041-011` and `firmware_origin_fork: tumoflip`. |
...
| Build metadata | Uses upstream build metadata conventions. | Uses `t-dev-089-041-011` for the installed firmware version and release artifact suffix, while keeping `tumoflip` as the fork origin. |
...
- `flipper-z-f7-update-t-dev-089-041-011.tgz`
...
```text
dist/f7-C/flipper-z-f7-update-t-dev-089-041-011.tgz
```

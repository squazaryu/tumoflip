...
- Exposes ARF Sub-GHz Full, ARF Frequency Analyzer, Sub-GHz RAW Edit, and
  KeeLoq Keystore Decryptor in `/ext/apps/ARF Tools`; the remaining isolated
  child FAPs are packaged under
...
| --- | --- | --- |
| Firmware identity | Reports itself as Unleashed. | Reports `firmware_version: tmwhflpprarf089-030` and `firmware_origin_fork: tumoflip`. |
...
| Build metadata | Uses upstream build metadata conventions. | Uses `tmwhflpprarf089-030` for the installed firmware version and release artifact suffix, while keeping `tumoflip` as the fork origin. |
...
- `flipper-z-f7-update-tmwhflpprarf089-030.tgz`
...
```text
dist/f7-C/flipper-z-f7-update-tmwhflpprarf089-030.tgz
```

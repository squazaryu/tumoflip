...
- `005`: development iteration inside the standalone Tumoflip release number.
...
| --- | --- | --- |
| Firmware identity | Uses another distribution identity. | Reports `firmware_version: t-dev-004-005` and `firmware_origin_fork: tumoflip`. |
...
| Build metadata | Uses upstream build metadata conventions. | Uses `t-dev-004-005` for the installed firmware version and release artifact suffix, while keeping `tumoflip` as the fork origin. |
...
Holding `OK` on the Time row in `Settings -> Clock` synchronizes the RTC with
the connected iPhone. Successful Sub-GHz, NFC, and LF RFID saves can receive
an optional transactional `.tumoflip.json` location sidecar; TumoSurvey, Field
Logger, and TumoSpectrum consume the same bounded phone service without adding
always-running GPS or network daemons to the firmware.
...
- `flipper-z-f7-update-t-dev-004-005.tgz`
...
```text
dist/f7-C/flipper-z-f7-update-t-dev-004-005.tgz
```

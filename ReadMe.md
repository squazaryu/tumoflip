...
- Release: `v0.3.4` published release (hardware validation in progress)
...
| --- | --- | --- |
| Firmware identity | Reports itself as Unleashed. | Reports `firmware_version: tmwhflpprarf089-034` and `firmware_origin_fork: tumoflip`. |
...
| Build metadata | Uses upstream build metadata conventions. | Uses `tmwhflpprarf089-034` for the installed firmware version and release artifact suffix, while keeping `tumoflip` as the fork origin. |
...
- `flipper-z-f7-update-tmwhflpprarf089-034.tgz`
...
```text
dist/f7-C/flipper-z-f7-update-tmwhflpprarf089-034.tgz
```

...
static bool ibutton_settings_write_targets_on_save(void) {
    // Opening a page must not overwrite an unreadable/newer-version settings file.
    // An explicit edit opts into replacing it; a failed save remains visible to the caller.
    return !page.dirty || ibutton_settings_set_write_targets(page.mask);
}

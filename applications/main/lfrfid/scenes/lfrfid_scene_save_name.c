        ...
        if(event.event == LfRfidEventNext) {
            ...
            if(saved) {
                if(had_file && !storage_common_equivalent_path(
                    ...
                    if(!lfrfid_reconcile_replaced_key(app, replaced_path)) {
                        FURI_LOG_E(TAG, "Failed to preserve replaced key metadata");
                    } else if(!lfrfid_delete_key_file(app, replaced_path)) {

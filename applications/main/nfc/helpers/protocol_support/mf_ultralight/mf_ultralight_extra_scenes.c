...
static bool nfc_mf_ultralight_aes_dict_attack_is_safe(const MfUltralightData* data) {
    if(!data || data->type != MfUltralightTypeUltralightAES ||
       data->pages_read <= MF_ULTRALIGHT_AES_ACCESS_PAGE) {
        return false;
    }
    const uint8_t* access = data->page[MF_ULTRALIGHT_AES_ACCESS_PAGE].data;
    return (access[2] | ((access[3] & 0x03U) << 8)) == 0;
}
...
static NfcCommand
    ...
    if(poller_event->type == MfUltralightPollerEventTypeRequestMode) {
        ...
        if(!nfc_mf_ultralight_aes_dict_attack_is_safe(data)) {
            view_dispatcher_send_custom_event(
                instance->view_dispatcher, NfcCustomEventDictAttackComplete);
...
// UL-AES has an AUTHLIM: failed keys can permanently lock the protected card.
static void mf_ultralight_scene_aes_dict_attack_warn_dialog_callback(
...
static bool mf_ultralight_scene_aes_dict_attack_warn_on_event(
    NfcApp* nfc,
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == DialogExResultRight) {
            const uint32_t next_scene = scene_manager_get_scene_state(

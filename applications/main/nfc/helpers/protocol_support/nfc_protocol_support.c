...
void nfc_protocol_support_alloc(NfcProtocol protocol, void* context) {
    ...
    if(protocol_support->base == &nfc_protocol_support_empty) {
        FURI_LOG_E(TAG, "Failed to load %s", plugin_path);
        ...
        if(event.event == NfcCustomEventTextInputDone) {
            ...
            if(saved) {
                ...
                if(had_file && !storage_common_equivalent_path(
                    ...
                    if(!nfc_reconcile_replaced_file(instance, replaced_path, instance->file_path)) {
                        FURI_LOG_E(TAG, "Failed to preserve replaced file metadata");
                    } else if(!nfc_delete_file(instance, replaced_path)) {

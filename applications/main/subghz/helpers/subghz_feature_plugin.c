...
const void* subghz_feature_plugin_load(
    ...
    *manager = plugin_manager_alloc(
    ...
    PluginManagerLoadStatus error = plugin_manager_load_single_detailed(*manager, path);
    if(error == PluginManagerLoadStatusSuccess) {
    ...
    if(error == PluginManagerLoadStatusNotEnoughMemory)
        missing_message = "Not enough RAM.\nClose other apps\nor reboot.";
    else if(error == PluginManagerLoadStatusMissingImports)
        missing_message = "Plugin needs another\nfirmware API. Update\nfirmware resources.";
    else if(error == PluginManagerLoadStatusTargetMismatch)
        missing_message = "Wrong hardware target.\nInstall F7 resources.";

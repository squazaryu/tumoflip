...
static uint32_t subghz_txrx_rx(SubGhzTxRx* instance, uint32_t frequency) {
    ...
    if(!subghz_radio_broker_session_open(
           ...
           "standard")) {
        FURI_LOG_W(TAG, "Unable to record Standard Sub-GHz RX session");
    } else {
...
static bool subghz_txrx_tx(SubGhzTxRx* instance, uint32_t frequency) {
    ...
    if(ret) {
        if(!subghz_radio_broker_session_open(
               instance->radio_broker, &instance->radio_lease, broker_device, "standard")) {
            FURI_LOG_W(TAG, "Unable to record Standard Sub-GHz TX session");
        } else {
            subghz_radio_broker_session_observe(
                instance->radio_broker,
                &instance->radio_lease,

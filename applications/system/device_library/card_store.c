...
static bool card_read(Storage* storage, const char* name, unsigned slot, CardEnvelope* e) {
    ...
    bool ok = storage_file_open(file, furi_string_get_cstr(path), FSAM_READ, FSOM_OPEN_EXISTING) &&
        ...
        e->magic == CARD_MAGIC && e->generation && library_card_valid(&e->card) && !strcmp(name, e->card.name);
    uint8_t hash[32];
    ok = ok && !mbedtls_sha256((const uint8_t*)e, offsetof(CardEnvelope, checksum), hash, 0) && !memcmp(hash, e->checksum, 32);
...
bool library_card_load(Storage* storage, const char* name, DeviceCard* card) {
    ...
    uint32_t best = 0;
    for(unsigned i = 0; i < LIBRARY_SLOTS; i++) if(card_read(storage, name, i, e) && e->generation > best) {
        *card = e->card;
        best = e->generation;
    }
    free(e);
...
bool library_card_save(Storage* storage, const DeviceCard* card) {
    if(!library_card_valid(card) || !storage_simply_mkdir(storage, LIBRARY_ROOT) || !storage_simply_mkdir(storage, LIBRARY_CARDS)) return false;
    ...
    do {
        ...
        uint32_t gen[4] = {0};
        for(unsigned i = 0; i < 4; i++) if(card_read(storage, card->name, i, e)) gen[i] = e->generation;
        ...
        e->card = *card;
        if(mbedtls_sha256((const uint8_t*)e, offsetof(CardEnvelope, checksum), e->checksum, 0)) break;
        ...
        ok = created && storage_file_write(file, e, sizeof(*e)) == sizeof(*e) && storage_file_sync(file);

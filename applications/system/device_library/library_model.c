...
static bool library_text_valid(const char* text, size_t size) {
    ...
    for(; *text; text++) if((unsigned char)*text < 32) return false;
    return true;
}
bool library_card_valid(const DeviceCard* card) {
    if(!card || !library_text_valid(card->name, sizeof(card->name)) ||
       ...
       !library_text_valid(card->tags, sizeof(card->tags)) || card->link_count > LIBRARY_LINKS) return false;
    for(unsigned i = 0; i < card->link_count; i++) {
        if(!memchr(card->links[i], 0, sizeof(card->links[i])) || !library_path_valid(card->links[i])) return false;
        for(unsigned j = 0; j < i; j++) if(!strcmp(card->links[i], card->links[j])) return false;
    }
    return true;
}
int library_newest_slot(const uint32_t gen[LIBRARY_SLOTS]) {
    int newest = -1;
    for(unsigned i = 0; i < LIBRARY_SLOTS; i++) if(gen[i] && (newest < 0 || gen[i] > gen[newest])) newest = i;
    return newest;
}
unsigned library_write_slot(const uint32_t gen[LIBRARY_SLOTS]) {
    unsigned slot = 0;
    for(unsigned i = 0; i < LIBRARY_SLOTS; i++) if(gen[i] < gen[slot]) slot = i;
    return slot;
}

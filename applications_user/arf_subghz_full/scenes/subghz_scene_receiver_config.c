...
enum SubGhzSettingIndex {
    ...
    SubGhzSettingIndexCombinedHopping,
...
static void subghz_scene_receiver_config_set_hopping(VariableItem* item) {
    ...
    variable_item_set_locked(
        ...
        index != 0,
        "Turn off\nHopping\nfirst!");
    VariableItem* combined_hopping_item =
        variable_item_list_get(subghz->variable_item_list, SubGhzSettingIndexCombinedHopping);
    variable_item_set_locked(
        combined_hopping_item,
        index != 0,
        "Turn off\nHopping\nfirst!");
}
...
static void subghz_scene_receiver_config_set_combined_hopping(VariableItem* item) {
    SubGhz* subghz = variable_item_get_context(item);
    ...
    variable_item_set_current_value_text(item, combobox_text[enabled]);
    subghz->last_settings->enable_combined_hopping = enabled;
    ...
    if(enabled) {
        ...
        subghz_txrx_preset_hopper_set_state(
            subghz->txrx, SubGhzPresetHopperStateRunning);
    } else {
        ...
        subghz_txrx_set_preset_internal(
            ...
            subghz->tx_power);
        char frequency_text[10];
        snprintf(
            frequency_text,
            sizeof(frequency_text),
            "%lu.%02lu",
            subghz->last_settings->frequency / 1000000,
            (subghz->last_settings->frequency % 1000000) / 10000);
        variable_item_set_current_value_text(frequency_item, frequency_text);
        variable_item_set_current_value_text(
            preset_item,
            subghz_setting_get_preset_name(setting, subghz->last_settings->preset_index));
    }

    variable_item_set_locked(hopping_item, enabled, "Turn off\nCombined\nHopping\nfirst!");
    variable_item_set_locked(
        preset_hopping_item, enabled, "Turn off\nCombined\nHopping\nfirst!");
}
...
static void subghz_scene_receiver_config_var_list_enter_callback(void* context, uint32_t index) {
    ...
    } else if(index == SubGhzSettingIndexProtocolPackInfo) {
...
void subghz_scene_receiver_config_on_enter(void* context) {
    ...
    if(scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
       SubGhzCustomEventManagerSet) {
        ...
        variable_item_set_locked(
            item,
            subghz->last_settings->enable_preset_hopping ||
        ...
        variable_item_set_locked(
            item,
            subghz->last_settings->enable_hopping ||
                subghz->last_settings->enable_combined_hopping,
            "Turn off\nHopping\nfirst!");
        ...
        item = variable_item_list_add(
            ...
            "Combined Hopping",
            COMBO_BOX_COUNT,
            subghz_scene_receiver_config_set_combined_hopping,
            subghz);
        value_index = subghz->last_settings->enable_combined_hopping;
        variable_item_set_current_value_index(item, value_index);

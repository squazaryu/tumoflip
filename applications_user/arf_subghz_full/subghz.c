...
#if defined(ARF_PROFILE_KEELOQ) || defined(ARF_PROFILE_COUNTER) || defined(ARF_PROFILE_CAR) || \
    defined(ARF_PROFILE_FA) || defined(ARF_PROFILE_PSA)
#define ARF_PROFILE_TOOL 1
#else
#define ARF_PROFILE_TOOL 0
#endif
...
static void __attribute__((unused))
    subghz_rpc_command_callback(const RpcAppSystemEvent* event, void* context) {
    furi_assert(context);
    SubGhz* subghz = context;
...
SubGhz* subghz_alloc(bool alloc_for_tx_only) {
    ...
    if(alloc_for_tx_only) {
        subghz_ensure_read_raw_view(subghz, true);
    }
#endif
    ...
    if(subghz_txrx_radio_device_get(subghz->txrx) == SubGhzRadioDeviceTypeInternal) {
        subghz_txrx_radio_device_set(subghz->txrx, SubGhzRadioDeviceTypeExternalCC1101);
    }
...
#if defined(ARF_PROFILE_KEELOQ)
    if(subghz->subghz_keeloq_decrypt) {

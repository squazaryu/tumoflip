...
#define SUBGHZ_LAST_SETTING_FIELD_COMBINED_HOPPING                  "CombinedHopping"
#define SUBGHZ_LAST_SETTING_FIELD_COMBINED_HOPPING_THRESHOLD        "CombinedHoppingThreshold"
    ...
    if(FSE_OK == storage_sd_status(storage) &&
       flipper_format_file_open_existing(fff_data_file, SUBGHZ_LAST_SETTINGS_PATH)) {
        do {
            ...
            if(!flipper_format_read_bool(
                   fff_data_file,
                   SUBGHZ_LAST_SETTING_FIELD_COMBINED_HOPPING,
                   &instance->enable_combined_hopping,
                   1)) {
                instance->enable_combined_hopping = false;
                flipper_format_rewind(fff_data_file);
            }
            if(!flipper_format_read_float(
                   fff_data_file,
                   SUBGHZ_LAST_SETTING_FIELD_COMBINED_HOPPING_THRESHOLD,
                   &instance->combined_hopping_threshold,
                   1)) {
                instance->combined_hopping_threshold =
                    SUBGHZ_LAST_SETTING_DEFAULT_COMBINED_HOPPING_THRESHOLD;
                flipper_format_rewind(fff_data_file);
            }
            if(!flipper_format_read_uint32(
                   fff_data_file,

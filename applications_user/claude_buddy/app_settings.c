...
BleMode app_settings_get_ble_mode(void) {
    /* Merged build: Bridge only. The Desktop/NUS mode is retired, so report
     * Bridge unconditionally — keeps the whole UI (labels, help, shift-tab)
     * consistent regardless of any stale persisted value. */
    return BleModeBridge;
}

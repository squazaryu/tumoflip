...
static Transport* transport_ble_alloc(void* app_ctx) {
    /* Merged build: always the standard-serial Bridge profile (never NUS), so
     * the iPhone passthrough link stays valid and control + transcript run in
     * one mode. The Desktop/NUS path is retired. */
    Transport* t = transport_bt_alloc();
...
static void app_on_transport_mode_changed(App* app, bool on_ble) {
    (void)on_ble;
    app->current_ble_mode = BleModeBridge; // merged build: Bridge only; Desktop branches are dead
...
/* Merged build: mirror host notify/status lines into the transcript ring so the
 * Info -> Transcript page shows a live activity log in Bridge mode too (this was
 * a Desktop-only view before). The ring buffer is always initialised + thread-safe. */
static void cb_transcript_log(const char* a, const char* b) {
    if(!a || !a[0]) return;
    char line[NUS_TRANSCRIPT_LINE_MAX];
    ...
    /* Skip consecutive duplicates so a repeated event doesn't flood the log. */
    static char last[NUS_TRANSCRIPT_LINE_MAX];
...
static void process_message(App* app, ProtocolMessage* msg) {
    ...
    case MsgTypeNotify: {
        ...
        if(msg->text[0] && !is_voice) {
            ...
            cb_transcript_log(msg->text, msg->text2);
        }
        break;
    }
    ...
    case MsgTypeStatus:
        ...
        ui_show_status2(
            ...
            true);
        /* Status ("Thinking…") is the live working pulse shown on the main
         * screen — deliberately NOT logged to the transcript, which is an
         * event log fed only by notify-with-text below. */
        break;
    ...
    case MsgTypeTokens:
        /* Persistent session-token line on the main screen (Bridge). */
        ui_set_tokens(app->ui, msg->text);
        break;

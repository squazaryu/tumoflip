...
static void status_draw(Canvas* canvas, void* model) {
    ...
    if(desktop) {
        ...
        /* Line 2 = session tokens (persistent), falling back to a transient
         * subtext, then the menu hint. Tokens live in their own field so live
         * action updates (which clear subtext) don't wipe the counter. */
        const char* line2 = m->tokens[0] ? m->tokens : (m->subtext[0] ? m->subtext : NULL);
        bool show_hint = !line2 && m->connected && m->text[0] == '\0';

        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(
            canvas, 77, (line2 || show_hint) ? 25 : 31, AlignCenter, AlignCenter, main_text);
        if(line2) {
            canvas_set_font(canvas, FontSecondary);
            canvas_draw_str_aligned(canvas, 77, 37, AlignCenter, AlignCenter, line2);
        } else if(show_hint) {
            canvas_set_font(canvas, FontSecondary);
...
static bool info_menu_item_visible(int idx) {
    if(idx == INFO_IDX_BLE) return false; // merged build: no mode toggle (Bridge only)
...
void ui_set_tokens(UiState* ui, const char* tokens) {
    if(!ui) return;
    StatusModel* m = view_get_model(ui->status_view);
    if(tokens) {
        strncpy(m->tokens, tokens, sizeof(m->tokens) - 1);
        m->tokens[sizeof(m->tokens) - 1] = '\0';
    } else {
        m->tokens[0] = '\0';
    }
    view_commit_model(ui->status_view, true); // update only the token line; leave action/pose
}

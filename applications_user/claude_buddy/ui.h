...
typedef struct {
    ...
    char tokens[24]; // session token counter line (Bridge), persistent across actions
} StatusModel;
...
/* Set the persistent session-token line shown at the bottom of the Bridge status
 * screen. Independent of text/subtext so live actions don't clear it. */

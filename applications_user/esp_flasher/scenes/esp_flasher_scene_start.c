...
bool esp_flasher_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event != SubmenuIndexEspFlasherPackage) app->package_mode = false;
        if(event.event == SubmenuIndexEspFlasherAbout) {
            scene_manager_next_scene(app->scene_manager, EspFlasherSceneAbout);
            consumed = true;
        } else if(event.event == SubmenuIndexEspFlasherPackage) {
            scene_manager_next_scene(app->scene_manager, EspFlasherScenePackageList);
            consumed = true;

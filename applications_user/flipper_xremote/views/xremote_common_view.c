...
static const XRemoteButton g_buttons[XREMOTE_BUTTON_COUNT + 1] = {
    ...
    {27, XREMOTE_COMMAND_OFF},
    {28, XREMOTE_COMMAND_DRY},
    {29, XREMOTE_COMMAND_COOL_HI},
    {30, XREMOTE_COMMAND_COOL_LO},
    {31, XREMOTE_COMMAND_HEAT_HI},
    {32, XREMOTE_COMMAND_HEAT_LO},
    {33, XREMOTE_COMMAND_POWER_ON},
    {34, XREMOTE_COMMAND_POWER_OFF},
    {35, XREMOTE_COMMAND_BRIGHTNESS_UP},
    {36, XREMOTE_COMMAND_BRIGHTNESS_DN},
    {37, XREMOTE_COMMAND_RED},
    {38, XREMOTE_COMMAND_GREEN},
    {39, XREMOTE_COMMAND_BLUE},
    {40, XREMOTE_COMMAND_WHITE},
    {41, XREMOTE_COMMAND_SPEED_UP},
    {42, XREMOTE_COMMAND_SPEED_DN},
    {43, XREMOTE_COMMAND_ROTATE},
    {44, XREMOTE_COMMAND_TIMER},
    {45, XREMOTE_COMMAND_SMART},
    {46, XREMOTE_COMMAND_TEMP_UP},
    {47, XREMOTE_COMMAND_TEMP_DOWN},
    {48, XREMOTE_COMMAND_SUPER},
    {49, XREMOTE_COMMAND_FAN_SPEED},
    {50, XREMOTE_COMMAND_I_FEEL},
    {51, XREMOTE_COMMAND_SLEEP},
    {52, XREMOTE_COMMAND_SWING_V},
    {53, XREMOTE_COMMAND_SWING_H},
    {54, XREMOTE_COMMAND_CLOCK},
    {55, XREMOTE_COMMAND_TIMER_ON},
    {56, XREMOTE_COMMAND_TIMER_OFF},
    {57, XREMOTE_COMMAND_QUIET},
    {58, XREMOTE_COMMAND_DIMMER},
    {59, XREMOTE_COMMAND_ECONOMY},
    {-1, NULL}};

typedef struct {
    XRemoteRemoteType type;
    const char* name;
    const char* menu_name;
    const char* const* buttons;
    uint32_t button_count;
} XRemoteRemoteTypeInfo;

static const char* const g_generic_buttons[] = {
    XREMOTE_COMMAND_POWER,
    XREMOTE_COMMAND_SETUP,
    XREMOTE_COMMAND_INPUT,
    XREMOTE_COMMAND_EJECT,
    XREMOTE_COMMAND_MENU,
    XREMOTE_COMMAND_LIST,
    XREMOTE_COMMAND_INFO,
    ...
    XREMOTE_COMMAND_STOP,
    XREMOTE_COMMAND_MUTE,
    XREMOTE_COMMAND_MODE,
...
static const char* const g_tv_buttons[] = {
    XREMOTE_COMMAND_POWER,
    XREMOTE_COMMAND_MUTE,
    XREMOTE_COMMAND_VOL_UP,
    XREMOTE_COMMAND_NEXT_CHAN,
...
static const char* const g_audio_buttons[] = {
    XREMOTE_COMMAND_POWER,
    XREMOTE_COMMAND_MUTE,
    XREMOTE_COMMAND_PLAY,
    XREMOTE_COMMAND_PAUSE,
...
static const char* const g_projector_buttons[] = {
    XREMOTE_COMMAND_POWER,
    XREMOTE_COMMAND_MUTE,
    XREMOTE_COMMAND_VOL_UP,
    XREMOTE_COMMAND_VOL_DOWN,
...
static const char* const g_leds_buttons[] = {
    XREMOTE_COMMAND_POWER_ON,
    XREMOTE_COMMAND_POWER_OFF,
    XREMOTE_COMMAND_BRIGHTNESS_UP,
    XREMOTE_COMMAND_BRIGHTNESS_DN,
    XREMOTE_COMMAND_RED,
    XREMOTE_COMMAND_GREEN,
    XREMOTE_COMMAND_BLUE,
    XREMOTE_COMMAND_WHITE,
};

static const char* const g_fan_buttons[] = {
    XREMOTE_COMMAND_POWER,
    XREMOTE_COMMAND_MODE,
    XREMOTE_COMMAND_SPEED_UP,
    XREMOTE_COMMAND_SPEED_DN,
    XREMOTE_COMMAND_ROTATE,
    XREMOTE_COMMAND_TIMER,
};

static const char* const g_ac_buttons[] = {
    XREMOTE_COMMAND_POWER,
    XREMOTE_COMMAND_SMART,
    XREMOTE_COMMAND_MODE,
    XREMOTE_COMMAND_TEMP_UP,
    XREMOTE_COMMAND_TEMP_DOWN,
    XREMOTE_COMMAND_SUPER,
    XREMOTE_COMMAND_FAN_SPEED,
    XREMOTE_COMMAND_I_FEEL,
    XREMOTE_COMMAND_SLEEP,
    XREMOTE_COMMAND_SWING_V,
    XREMOTE_COMMAND_SWING_H,
    XREMOTE_COMMAND_CLOCK,
    XREMOTE_COMMAND_TIMER_ON,
    XREMOTE_COMMAND_TIMER_OFF,
    XREMOTE_COMMAND_QUIET,
    XREMOTE_COMMAND_DIMMER,
    XREMOTE_COMMAND_ECONOMY,
};
...
static const XRemoteRemoteTypeInfo g_remote_types[XRemoteRemoteTypeCount] = {
    ...
    {XRemoteRemoteTypeLEDs, "LEDs", "LEDs", g_leds_buttons, COUNT_OF(g_leds_buttons)},
    {XRemoteRemoteTypeFan, "Fan", "Fan", g_fan_buttons, COUNT_OF(g_fan_buttons)},
    {XRemoteRemoteTypeAC, "AC", "A/C", g_ac_buttons, COUNT_OF(g_ac_buttons)},
};

const char* xremote_button_get_name(int index) {
    if(index < 0 || index >= XREMOTE_BUTTON_COUNT) return NULL;
...
const char* xremote_remote_type_get_name(XRemoteRemoteType type) {
    if(type >= XRemoteRemoteTypeCount) type = XRemoteRemoteTypeGeneric;
    return g_remote_types[type].name;
}

const char* xremote_remote_type_get_menu_name(XRemoteRemoteType type) {
    if(type >= XRemoteRemoteTypeCount) type = XRemoteRemoteTypeGeneric;
    return g_remote_types[type].menu_name;
}

XRemoteRemoteType xremote_remote_type_from_name(const char* name) {
    if(name == NULL) return XRemoteRemoteTypeGeneric;

    for(uint32_t i = 0; i < XRemoteRemoteTypeCount; i++) {
        if(!strcmp(name, g_remote_types[i].name)) return g_remote_types[i].type;
    }

    return XRemoteRemoteTypeGeneric;
}

uint32_t xremote_remote_type_get_button_count(XRemoteRemoteType type) {
    if(type >= XRemoteRemoteTypeCount) type = XRemoteRemoteTypeGeneric;
    return g_remote_types[type].button_count;
}

const char* xremote_remote_type_get_button_name(XRemoteRemoteType type, uint32_t index) {
    if(type >= XRemoteRemoteTypeCount) type = XRemoteRemoteTypeGeneric;
    if(index >= g_remote_types[type].button_count) return NULL;
    return g_remote_types[type].buttons[index];
}
...
int32_t xremote_remote_type_get_button_index(XRemoteRemoteType type, const char* name) {
    ...
    for(uint32_t i = 0; i < g_remote_types[type].button_count; i++) {
        if(!strcmp(name, g_remote_types[type].buttons[i])) return i;
    }
...
void xremote_canvas_draw_button(
    ...
    uint8_t y,
    XRemoteIcon icon) {
    canvas_draw_icon(canvas, x, y, &I_Xc_Button_18x18);

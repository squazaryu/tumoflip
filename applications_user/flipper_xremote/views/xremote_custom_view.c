...
static void xremote_custom_view_draw_page_name(
    Canvas* canvas,
    ...
    const char* page_name = remote_type == XRemoteRemoteTypeGeneric ?
                                "Custom" :
                                xremote_remote_type_get_menu_name(remote_type);
    elements_multiline_text_aligned(canvas, x, y, align, AlignTop, page_name);
}
...
static void xremote_custom_view_draw_callback(Canvas* canvas, void* context) {
    ...
    xremote_custom_view_draw_page_name(canvas, orientation, buttons->remote_type);

...
#include "views/xremote_universal_view.h"
...
static uint32_t xremote_control_direct_view_exit_callback(void* context) {
    UNUSED(context);
    return XRemoteViewSubmenu;
}
...
static void xremote_control_submenu_callback(void* context, uint32_t index) {
    ...
    if(index == XRemoteViewIRGeneral)
        ...
        xremote_app_view_alloc2(
            app,
            index,
            ((XRemoteAppButtons*)app->context)->remote_type == XRemoteRemoteTypeGeneric ?
                xremote_custom_view_alloc :
                xremote_universal_view_alloc,
            app->context);
    ...
    if(app->view_ctx != NULL) {
        if(index != XRemoteViewIRCustomEditPage) {
            ...
            if(((XRemoteAppButtons*)app->context)->remote_type == XRemoteRemoteTypeGeneric) {
...
XRemoteApp* xremote_control_alloc(XRemoteAppContext* app_ctx) {
    ...
    if(buttons->remote_type == XRemoteRemoteTypeGeneric) {
        ...
        xremote_app_view_alloc2(
            app, XRemoteViewIRSubmenu, xremote_universal_view_alloc, app->context);
        xremote_app_view_set_previous_callback(app, xremote_control_direct_view_exit_callback);
    }

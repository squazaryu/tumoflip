...
static void xremote_item_update_item(VariableItem* item, FuriString* button) {
    ...
    const char* button_name =
        xremote_remote_type_get_button_name(buttons->remote_type, button_index);
    if(button_name == NULL) button_name = xremote_button_get_name(button_index);
...
static void xremote_edit_list_add_item(
    ...
    uint32_t button_count = xremote_remote_type_get_button_count(context->buttons->remote_type);
    item = variable_item_list_add(list, item_name, button_count, change_callback, context);

    /* Get button name and index */
    const char* button_name = furi_string_get_cstr(button);
    int32_t button_index =
        xremote_remote_type_get_button_index(context->buttons->remote_type, button_name);
    if(button_index < 0) button_index = 0;
    button_name = xremote_remote_type_get_button_name(context->buttons->remote_type, button_index);

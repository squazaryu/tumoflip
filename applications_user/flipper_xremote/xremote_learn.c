...
const char* xremote_learn_get_curr_button_name(XRemoteLearnContext* learn_ctx) {
    ...
    return xremote_remote_type_get_button_name(learn_ctx->remote_type, learn_ctx->current_button);
}
...
XRemoteRemoteType xremote_learn_get_remote_type(XRemoteLearnContext* learn_ctx) {
    xremote_app_assert(learn_ctx, XRemoteRemoteTypeGeneric);
    return learn_ctx->remote_type;
}
...
static uint32_t xremote_learn_view_exit_callback(void* context) {
    UNUSED(context);
    return XRemoteViewLearnCapture;
}
...
static uint32_t xremote_learn_text_input_exit_callback(void* context) {
    ...
    const uint32_t button_count = xremote_remote_type_get_button_count(learn_ctx->remote_type);
    if(learn_ctx->current_button >= button_count) learn_ctx->current_button = button_count - 1;
...
static void xremote_learn_text_input_callback(void* context) {
    ...
    if(infrared_remote_get_button_count(learn_ctx->ir_remote) &&
       learn_ctx->text_store[0] != '\0') {
        ...
        buttons->app_ctx = learn_ctx->app_ctx;
        buttons->remote_type = learn_ctx->remote_type;
        xremote_app_buttons_set_remote_type(buttons, learn_ctx->remote_type);
...
static bool xremote_learn_custom_event_callback(void* context, uint32_t event) {
    ...
    if(event == XRemoteEventSignalReceived) {
        ...
        if(++learn_ctx->current_button >=
           xremote_remote_type_get_button_count(learn_ctx->remote_type)) {
            xremote_learn_context_rx_stop(learn_ctx);
        ...
        xremote_learn_switch_to_view(learn_ctx, XRemoteViewLearnCapture);
...
static XRemoteLearnContext* xremote_learn_context_alloc(XRemoteAppContext* app_ctx) {
    ...
    learn_ctx->text_store[0] = 0;
    learn_ctx->current_button = 0;
    learn_ctx->remote_type = XRemoteRemoteTypeGeneric;
...
static uint32_t xremote_learn_type_exit_callback(void* context) {
    UNUSED(context);
    return XRemoteViewSubmenu;
}

static void xremote_learn_type_submenu_callback(void* context, uint32_t index) {
    furi_assert(context);
    XRemoteApp* app = context;
    XRemoteLearnContext* learn = app->context;
    xremote_app_assert_void(learn);

    if(index >= XRemoteRemoteTypeCount) index = XRemoteRemoteTypeGeneric;
    learn->remote_type = index;
    learn->current_button = 0;
    infrared_remote_reset(learn->ir_remote);
    xremote_learn_context_rx_start(learn);
    xremote_learn_switch_to_view(learn, XRemoteViewLearnCapture);
}

XRemoteApp* xremote_learn_alloc(XRemoteAppContext* app_ctx) {
    XRemoteApp* app = xremote_app_alloc(app_ctx);
    xremote_app_submenu_alloc(app, XRemoteViewLearn, xremote_learn_type_exit_callback);

    XRemoteLearnContext* learn = xremote_learn_context_alloc(app_ctx);
    xremote_app_set_user_context(app, learn, xremote_learn_context_clear_callback);

    for(uint32_t i = 0; i < XRemoteRemoteTypeCount; i++) {
        xremote_app_submenu_add(
            app, xremote_remote_type_get_menu_name(i), i, xremote_learn_type_submenu_callback);
    }

    app->view_id = XRemoteViewLearnCapture;
    ...
    xremote_app_set_view_context(app, learn, NULL);

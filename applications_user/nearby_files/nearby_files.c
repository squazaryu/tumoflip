...
NearbyFilesApp* nearby_files_app_alloc(void) {
    ...
    // Prefer the phone bridge so a separate GPS module is not required. UART
    // NMEA remains available from the GPS Source menu for standalone use.
    GpsProtocol config_protocol = GpsProtocolRpc;
    ...
    widget_add_text_scroll_element(
        ...
        "current location.\n"
        "GPS from a UART module or\n"
        "TumoCompanion over BLE\n"
    ...
    if(coords.valid) {
        ...
        if(!coords.module_detected) {
            widget_add_string_element(
                ...
                AlignCenter,
                FontPrimary,
                rpc_source ? "No TumoCompanion" : "No GPS Module");
            widget_add_string_element(
                app->widget,
                64,
                40,
                ...
                AlignCenter,
                FontSecondary,
                rpc_source ? "Open TumoCompanion" : "Please connect GPS module");
        } else {

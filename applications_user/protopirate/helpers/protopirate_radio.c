...
bool protopirate_radio_init(ProtoPirateApp* app) {
    ...
    if(!subghz_radio_broker_acquire(
           app->radio_broker, "proto_pirate", FuriWaitForever, &app->radio_lease)) {
    ...
    app->txrx->radio_device = radio_device_loader_set(
        NULL, SubGhzRadioDeviceTypeExternalCC1101, app->radio_broker, &app->radio_lease);

    if(!app->txrx->radio_device) {
        FURI_LOG_W(TAG, "External CC1101 not found, trying internal radio");
        app->txrx->radio_device = radio_device_loader_set(
            NULL, SubGhzRadioDeviceTypeInternal, app->radio_broker, &app->radio_lease);
    }

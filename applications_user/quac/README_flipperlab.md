...
* Playback of Sub-GHz, RFID, IR, NFC, non-secure Picopass, and iButton signals
* Easy navigation
...
The signal files are played back as recorded. During playback/transmit, the LED light will flash blue until the action is complete. For SubGhz (non-RAW), RFID, NFC, Picopass, and iButton signals, they are continuously played back for the durations specified in the Settings. Picopass is playback-only and accepts only saved non-secure, unencrypted credentials. Secure/encrypted or key-bearing files are rejected before NFC starts. Press Back to cancel.
...
* Picopass Duration: Changes the bounded emulation duration for an existing
  non-secure `.picopass` credential. A playlist override must be 100 through
  10000 ms.

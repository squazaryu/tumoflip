#include "rolljam_psa_bf_plugin.h"
...
#ifdef ROLLJAM_PSA_BF_PLUGIN_BUILD
#include "rolljam_psa_bf_plugin_icons.h"
#else
...
static const RollJamPsaBfHostApi* g_host_api = NULL;

static PsaBfState* g_bf_state = NULL;
static FuriThread* g_bf_thread = NULL;
static RollJamPsaBfContext g_active_ctx = RollJamPsaBfContextReceiverInfo;
...
static void psa_bf_done_cb(void* context) {
    if(g_host_api && g_host_api->send_custom_event) {
        g_host_api->send_custom_event(
            context, RollJamCustomEventPsaBruteforceComplete);
    }
}
...
static void bf_result_ok_callback(GuiButtonType result, InputType type, void* context) {
    ...
    if((type == InputTypeShort || type == InputTypeLong) && result == GuiButtonTypeCenter) {
        if(g_host_api && g_host_api->send_custom_event) {
            g_host_api->send_custom_event(
                app, RollJamCustomEventReceiverInfoBruteforceCancel);
        }
    }
}
...
static void bf_finish_and_show_result(void* app, ButtonCallback result_callback) {
    ...
    if(status == PSA_BF_STATUS_FOUND) {
        ...
        if(g_active_ctx == RollJamPsaBfContextSubDecode) {
            g_host_api->notification_success(app);
        }
        ButtonCallback ok_cb = result_callback;
        if(!ok_cb && g_active_ctx == RollJamPsaBfContextReceiverInfo) {
...
static bool plugin_needs_bruteforce(void* app, RollJamPsaBfContext ctx) {
    FlipperFormat* ff = g_host_api->get_history_flipper_format(app);
    return item_needs_bruteforce_from_ff(
        ff, ctx == RollJamPsaBfContextReceiverInfo);
}
...
static void plugin_on_scene_enter(void* app, RollJamPsaBfContext ctx) {
    g_active_ctx = ctx;
...
static bool plugin_on_scene_event(
    ...
    if(event.event == RollJamCustomEventPsaBruteforceComplete) {
        if(g_bf_state) {
            bf_finish_and_show_result(app, NULL);
            if(g_active_ctx == RollJamPsaBfContextSubDecode) {
    ...
    if(ctx == RollJamPsaBfContextReceiverInfo) {
        if(event.event == RollJamCustomEventReceiverInfoBruteforceStart) {
            return start_bruteforce(app);
        }
        if(event.event == RollJamCustomEventReceiverInfoBruteforceCancel) {
            if(g_bf_state && g_bf_state->status == PSA_BF_STATUS_FOUND) {
                g_host_api->receiver_info_rebuild_widget(app);
                free(g_bf_state);
                g_bf_state = NULL;
    ...
    if(ctx == RollJamPsaBfContextSubDecode) {
        if(event.event == RollJamCustomEventSubDecodeBruteforceStart) {
...
static void plugin_on_scene_exit(void* app, RollJamPsaBfContext ctx) {
    UNUSED(app);
    UNUSED(ctx);
}

static bool plugin_widget_left_should_bruteforce(void* app, RollJamPsaBfContext ctx) {
...
static void plugin_set_host_api(const RollJamPsaBfHostApi* api) {
    g_host_api = api;
}

const RollJamPsaBfPlugin rolljam_psa_bf_plugin = {
    .plugin_name = "RollJam PSA BF",
...
static const FlipperAppPluginDescriptor rolljam_psa_bf_plugin_descriptor = {
    .appid = ROLLJAM_PSA_BF_PLUGIN_APP_ID,
    .ep_api_version = ROLLJAM_PSA_BF_PLUGIN_API_VERSION,
    .entry_point = &rolljam_psa_bf_plugin,
};

const FlipperAppPluginDescriptor* rolljam_psa_bf_plugin_ep(void) {
    return &rolljam_psa_bf_plugin_descriptor;
}

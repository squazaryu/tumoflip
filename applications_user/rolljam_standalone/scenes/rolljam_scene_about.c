...
static void about_draw_callback(Canvas* canvas, void* context) {
    ...
    canvas_draw_str(canvas, x_off, 10, "RollJam v" FAP_VERSION);
    ...
    if(event->key == expected) {
        ...
        if(g_state.combo_progress >= EMULATE_TOGGLE_COMBO_LEN) {
            ...
            view_dispatcher_send_custom_event(
                app->view_dispatcher, RollJamCustomEventAboutToggleEmulate);
        }
...
static void about_show_emulate_toggle_popup(RollJamApp* app) {
...
void rolljam_scene_about_on_enter(void* context) {
    furi_check(context);
    RollJamApp* app = context;

    if(!rolljam_ensure_view_about(app)) {
...
bool rolljam_scene_about_on_event(void* context, SceneManagerEvent event) {
    RollJamApp* app = context;
    bool consumed = false;
    ...
    if(event.type == SceneManagerEventTypeTick) {
        ...
        if(event.event == RollJamCustomEventAboutToggleEmulate) {
...
void rolljam_scene_about_on_exit(void* context) {
    furi_check(context);
    RollJamApp* app = context;

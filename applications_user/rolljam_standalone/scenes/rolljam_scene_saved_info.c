// scenes/rolljam_scene_saved_info.c
#include "../rolljam_app_i.h"
#include "../helpers/rolljam_storage.h"
#include "../protocols/protocols_common.h"
//#include "rolljam_standalone_icons.h"

#define TAG "RollJamSceneSavedInfo"

static void rolljam_scene_saved_info_widget_callback(
    ...
    if((result == GuiButtonTypeLeft) && (type == InputTypeShort)) {
        ...
        if(app->emulate_feature_enabled && !app->emulate_disabled_for_loaded) {
            view_dispatcher_send_custom_event(
                app->view_dispatcher, RollJamCustomEventSavedInfoEmulate);
        }
#endif
    } else if(result == GuiButtonTypeRight && (type == InputTypeShort)) {
        //Send delete event and get user confirmation to delete.
        view_dispatcher_send_custom_event(
            app->view_dispatcher, RollJamCustomEventSavedInfoDelete);
    }
}

void rolljam_scene_saved_info_on_enter(void* context) {
    furi_check(context);
    RollJamApp* app = context;
    ...
    if(success && info_str && furi_string_size(info_str) > 0) {
        ...
        if(app->emulate_feature_enabled && !app->emulate_disabled_for_loaded) {
            widget_add_button_element(
                ...
                GuiButtonTypeLeft,
                "Emulate",
                rolljam_scene_saved_info_widget_callback,
                app);
        }
#endif
        widget_add_button_element(
...
bool rolljam_scene_saved_info_on_event(void* context, SceneManagerEvent event) {
    RollJamApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == RollJamCustomEventSavedInfoDelete) {
            ...
            if(app->loaded_file_path && !furi_string_empty(app->loaded_file_path)) {
                ...
                dialog_message_set_icon(message, NULL, 0, 12);
                ...
                if(dialog_result == DialogMessageButtonLeft) {
                    ...
                    rolljam_storage_delete_file(furi_string_get_cstr(app->loaded_file_path));
                    scene_manager_previous_scene(app->scene_manager);
                }
            }
            consumed = true;
...
#ifdef ENABLE_EMULATE_FEATURE
        if(event.event == RollJamCustomEventSavedInfoEmulate &&
...
void rolljam_scene_saved_info_on_exit(void* context) {
    RollJamApp* app = context;

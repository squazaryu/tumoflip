// scenes/rolljam_scene_shield_receiver_config.c
#include "../rolljam_app_i.h"

#ifdef ENABLE_SHIELD_RX_SCENE

#define TAG "RollJamSceneShieldCfg"
...
typedef enum {
    RollJamShieldConfigFrequency,
    RollJamShieldConfigPreset,
    RollJamShieldConfigTxOffset,
    RollJamShieldConfigTxPower,
} RollJamShieldConfigItem;

static uint8_t rolljam_shield_config_freq_index(RollJamApp* app, uint32_t frequency) {
...
static void rolljam_shield_config_set_freq_text(VariableItem* item, uint32_t frequency) {
    char text_buf[10] = {0};
    snprintf(
        text_buf,
        sizeof(text_buf),
...
static void rolljam_shield_config_set_frequency(VariableItem* item) {
    RollJamApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
    app->shield_freq = subghz_setting_get_frequency(app->setting, index);
    rolljam_shield_config_set_freq_text(item, app->shield_freq);
}

static void rolljam_shield_config_set_preset(VariableItem* item) {
    RollJamApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
static void rolljam_shield_config_set_tx_offset(VariableItem* item) {
    RollJamApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
static void rolljam_shield_config_set_tx_power(VariableItem* item) {
    RollJamApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
void rolljam_scene_shield_receiver_config_on_enter(void* context) {
    furi_check(context);
    ...
    item = variable_item_list_add(
        ...
        rolljam_shield_config_set_frequency,
        app);
    variable_item_set_current_value_index(
        item, rolljam_shield_config_freq_index(app, app->shield_freq));
    rolljam_shield_config_set_freq_text(item, app->shield_freq);
    ...
    item = variable_item_list_add(
        ...
        rolljam_shield_config_set_preset,
        app);
    ...
    item = variable_item_list_add(
        ...
        rolljam_shield_config_set_tx_offset,
        app);
    ...
    item = variable_item_list_add(
        ...
        rolljam_shield_config_set_tx_power,
        app);
...
bool rolljam_scene_shield_receiver_config_on_event(void* context, SceneManagerEvent event) {
    UNUSED(context);
    UNUSED(event);
    return false;
}

void rolljam_scene_shield_receiver_config_on_exit(void* context) {
    furi_check(context);
    RollJamApp* app = context;
    variable_item_list_reset(app->variable_item_list);
}

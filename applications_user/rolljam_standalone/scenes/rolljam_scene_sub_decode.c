// scenes/rolljam_scene_sub_decode.c
...
static void rolljam_scene_sub_decode_update_receiver_statusbar(
    RollJamApp* app,
    RollJamHistory* history) {
    ...
    rolljam_history_format_status_text(history, history_stat_str, sizeof(history_stat_str));
...
void rolljam_subdecode_psa_bf_complete_refresh(void* app) {
    ...
    view_dispatcher_send_custom_event(
        a->view_dispatcher, RollJamCustomEventSubDecodeUpdate);
}

// Forward declaration
static void rolljam_scene_sub_decode_widget_callback(
...
static void
    rolljam_scene_sub_decode_receiver_callback(RollJamCustomEvent event, void* context);

// Callback when receiver successfully decodes a signal
static void rolljam_sub_decode_receiver_callback(
    ...
    if(rolljam_history_add_to_history(
           ctx->history,
           decoder_base,
           app->txrx->preset,
        ...
        view_dispatcher_send_custom_event(
            app->view_dispatcher, RollJamCustomEventSubDecodeUpdate);
    }
...
static void rolljam_decode_draw_callback(Canvas* canvas, void* context) {
    UNUSED(context);
    SubDecodeContext* ctx = g_decode_ctx;
    if(!ctx) return;
...
static bool rolljam_decode_input_callback(InputEvent* event, void* context) {
    UNUSED(context);
...
static void rolljam_scene_sub_decode_widget_callback(
    ...
    if(type == InputTypeShort || type == InputTypeLong) {
        if(result == GuiButtonTypeRight) {
            ...
            view_dispatcher_send_custom_event(
                app->view_dispatcher, RollJamCustomEventSubDecodeSave);
        } else if(result == GuiButtonTypeLeft) {
            view_dispatcher_send_custom_event(
                app->view_dispatcher, RollJamCustomEventSubDecodeBruteforceStart);
        }
    }
}

void rolljam_scene_sub_decode_on_enter(void* context) {
    RollJamApp* app = context;

    if(!rolljam_ensure_receiver_view(app) || !rolljam_ensure_widget(app) ||
       !rolljam_ensure_view_about(app)) {
    ...
    //rolljam_history_reset(g_decode_ctx->history);
    g_decode_ctx->match_count = 0;
    g_decode_ctx->selected_history_index = 0;
    g_decode_ctx->raw_reader = NULL;

    rolljam_view_receiver_set_sub_decode_mode(app->rolljam_receiver, true);
    ...
    if(dialog_file_browser_show(
        ...
        view_set_draw_callback(app->view_about, rolljam_decode_draw_callback);
        view_set_input_callback(app->view_about, rolljam_decode_input_callback);
...
bool rolljam_scene_sub_decode_on_event(void* context, SceneManagerEvent event) {
    RollJamApp* app = context;
    bool consumed = false;
    SubDecodeContext* ctx = g_decode_ctx;
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == RollJamCustomEventSubDecodeUpdate) {
            ...
            if(ctx->state == DecodeStateDecodingRaw) {
                ...
                uint16_t history_count = rolljam_history_get_item(ctx->history);
                if(history_count > 0) {
                    rolljam_view_receiver_sync_menu_from_history(
                        app->rolljam_receiver, ctx->history);

                    rolljam_view_receiver_set_idx_menu(
                        app->rolljam_receiver, ctx->selected_history_index);

                    rolljam_scene_sub_decode_update_receiver_statusbar(app, ctx->history);
                }
            }
            consumed = true;
        } else if(event.event == RollJamCustomEventSubDecodeSave) {
            // Save the file (same as receiver_info)
            FlipperFormat* ff =
                rolljam_history_get_raw_data(ctx->history, ctx->selected_history_index);
            ...
            consumed = true;
        } else if(event.event == RollJamCustomEventSubDecodeBruteforceStart) {
...
//             }
        ...
        } else if(event.event == RollJamCustomEventPsaBruteforceComplete) {
            app->txrx->idx_menu_chosen = ctx->selected_history_index;
            rolljam_selected_capture_set(
                app,
                ctx->history,
...
//             }
        ...
        } else if(event.event == RollJamCustomEventViewReceiverOK) {
            ...
            consumed = true;
        } else if(event.event == RollJamCustomEventViewReceiverBack) {
            // User pressed back from history - reset and go to main menu
            rolljam_history_reset(ctx->history);
...
//         }
        ...
        case DecodeStateStartingWorker: {
            ...
            do {
                ...
                if(!rolljam_refresh_protocol_registry(app, true) || !app->txrx->receiver) {
                    FURI_LOG_E(TAG, "Failed to rebuild receiver for preset %s", preset_name_short);
                    break;
                }

                subghz_receiver_set_rx_callback(
                    app->txrx->receiver, rolljam_sub_decode_receiver_callback, app);
                ...
                if(history_count > 0) {
                    ...
                    if(ctx->can_save) {
                        widget_add_button_element(
                            ...
                            GuiButtonTypeRight,
                            "Save",
                            rolljam_scene_sub_decode_widget_callback,
                            app);
                    }
        ...
        case DecodeStateShowHistory: {
            ...
            if(history_count > 0) {
                ...
                rolljam_view_receiver_set_callback(
                    app->rolljam_receiver,
                    rolljam_scene_sub_decode_receiver_callback,
                    app);
        ...
        case DecodeStateShowSignalInfo: {
            ...
            if(ctx->selected_history_index < history_count) {
                ...
                /*rolljam_history_get_text_item_menu(
...
//                         app, RollJamPsaBfContextSubDecode);
//                 }
//                 if(needs_bf) {
//                     widget_add_button_element(
//                         app->widget,
...
void rolljam_scene_sub_decode_on_exit(void* context) {
    RollJamApp* app = context;
...
//     rolljam_psa_bf_context_release(app);
    ...
    if(app->txrx->history) {
        ...
        rolljam_history_reset(app->txrx->history);

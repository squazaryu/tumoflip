// scenes/rolljam_scene_timing_tuner.c
...
typedef struct {
    ...
    const RollJamProtocolTiming* timing_info;
...
static void timing_tuner_rx_callback(
    ...
    ctx->timing_info = rolljam_get_protocol_timing(protocol_name);
...
void rolljam_scene_timing_tuner_on_enter(void* context) {
    furi_check(context);
    RollJamApp* app = context;
    ...
    rolljam_begin(app, app->txrx->preset->data);
...
bool rolljam_scene_timing_tuner_on_event(void* context, SceneManagerEvent event) {
    RollJamApp* app = context;
    bool consumed = false;
...
void rolljam_scene_timing_tuner_on_exit(void* context) {
    RollJamApp* app = context;

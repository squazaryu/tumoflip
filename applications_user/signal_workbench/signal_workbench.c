...
enum { SensorBegin=0x5000, SensorValue, SensorAnalyze=0x5010, SensorExport, SensorAbout, SensorDone };
static uint32_t tumospectrum_sensor_previous(void* context) { UNUSED(context); return TumoSpectrumViewSensorMenu; }
static uint32_t tumospectrum_sensor_parent(void* context) { UNUSED(context); return TumoSpectrumViewSetActions; }
...
static void tumospectrum_sensor_menu(TumoSpectrumApp* app) {
    ...
    submenu_set_header(app->sensor_menu,"Sensor measurements");
    for(unsigned i=0;i<4;i++) {
        char label[48];int32_t mag=abs(app->sensor_values[i]);
        if(app->sensor_entered & (1U<<i)) snprintf(label,sizeof(label),"%u %s: %s%ld.%ld",i+1,i==3?"Check":"Train",app->sensor_values[i]<0?"-":"",mag/10,mag%10);
        else snprintf(label,sizeof(label),"%u %s: not set",i+1,i==3?"Check":"Train");
        submenu_add_item(app->sensor_menu,label,SensorValue+i,tumospectrum_sensor_action,app);
    }
    submenu_add_item(app->sensor_menu,"Find hypotheses",SensorAnalyze,tumospectrum_sensor_action,app);
    submenu_add_item(app->sensor_menu,"Export report",SensorExport,tumospectrum_sensor_action,app);
    submenu_add_item(app->sensor_menu,"How to measure",SensorAbout,tumospectrum_sensor_action,app);
...
static bool tumospectrum_sensor_event(void* context,uint32_t event) {
    ...
    if(event==SensorBegin) {
        app->sensor_entered=0;memset(app->sensor_values,0,sizeof(app->sensor_values));
        furi_string_reset(app->sensor_report);tumospectrum_sensor_menu(app);
    } else if(event>=SensorValue && event<SensorValue+4) {
        app->sensor_selected=event-SensorValue;
        snprintf(app->sensor_header,sizeof(app->sensor_header),"Sample %u: value x10",app->sensor_selected+1);
        number_input_set_header_text(app->sensor_input,app->sensor_header);
        number_input_set_result_callback(app->sensor_input,tumospectrum_sensor_number,app,app->sensor_values[app->sensor_selected],-1000000,1000000);
        ...
        SensorObservation samples[4]={0};uint8_t bits=0;
        if(app->sensor_entered!=15 || !tumospectrum_sensor_decode(&app->capture_set,samples,&bits)) {
            tumospectrum_sensor_text(app,"Need four short, complete\nRAW files from one sensor,\nsame frequency / preset,\nand four measured values.\nNo hypothesis generated.");return true;
        }
        for(unsigned i=0;i<4;i++)samples[i].measured10=app->sensor_values[i];
        SensorFitResult result;
        if(!sensor_fit(samples,bits,&result)) {tumospectrum_sensor_text(app,"No matching hypothesis.\nUse three different\ntraining measurements.\nEncoding may be unsupported.");return true;}
        furi_string_printf(app->sensor_report,"Sensor hypotheses v1\n%lu fits; showing %u\n\nNot a proven decoder.\nFirst 3 samples train;\n4th is held out.\nValues below are x10.\n\n",result.total,result.count);
        for(unsigned i=0;i<4;i++) furi_string_cat_printf(app->sensor_report,"Sample %u: %ld\n%s\n",i+1,app->sensor_values[i],app->capture_set.samples[i].path);
        for(unsigned i=0;i<result.count;i++) {
            const SensorCandidate* c=&result.candidates[i];
            furi_string_cat_printf(app->sensor_report,"\n#%u Bits %u..%u\n%s / %s\nvalue10 = raw * %u\n%+ld\nCheck: %ld / %ld\n%s\n",i+1,c->start,c->start+c->width-1,c->signed_value?"signed":"unsigned",c->little_endian?"LE bytes":"MSB first",c->scale10,c->offset10,c->predicted10,app->sensor_values[3],c->holdout_match?"Held-out MATCH":"Held-out FAILED");
        }
        tumospectrum_sensor_text(app,furi_string_get_cstr(app->sensor_report));
    } else if(event==SensorExport) {
        if(furi_string_empty(app->sensor_report)){tumospectrum_sensor_text(app,"Analyze measurements first.");return true;}
        FuriString* path=furi_string_alloc();File* file=storage_file_alloc(app->storage);bool ok=false;
        for(unsigned i=0;i<1000;i++) {
            ...
            if(storage_file_open(file,furi_string_get_cstr(path),FSAM_WRITE,FSOM_CREATE_NEW)) {
                ok=storage_file_write(file,furi_string_get_cstr(app->sensor_report),furi_string_size(app->sensor_report))==furi_string_size(app->sensor_report) && storage_file_sync(file);
                ok=storage_file_close(file) && ok;if(!ok)storage_common_remove(app->storage,furi_string_get_cstr(path));break;
            }
            FS_Error error=storage_file_get_error(file);storage_file_close(file);if(error!=FSE_EXIST)break;
        }
        storage_file_free(file);
        if(ok)furi_string_printf(app->text,"Report saved:\n%s",furi_string_get_cstr(path));
        tumospectrum_sensor_text(app,ok?furi_string_get_cstr(app->text):"Report save failed.\nCheck SD / report limit.");
        furi_string_free(path);
    } else if(event==SensorAbout) tumospectrum_sensor_text(app,
...
static void tumospectrum_capture_set_build_actions(TumoSpectrumApp* app) {
    ...
    if(app->capture_set.type==TumoSpectrumCaptureSubGhzRaw && app->capture_set.sample_count==4)
        submenu_add_item(app->set_actions,"Sensor Helper",TumoSpectrumSetActionSensorHelper,tumospectrum_capture_set_action_callback,app);
...
static TumoSpectrumApp* tumospectrum_alloc(void) {
    ...
    app->sensor_report=furi_string_alloc();app->sensor_menu=submenu_alloc();app->sensor_input=number_input_alloc();
    view_set_previous_callback(submenu_get_view(app->sensor_menu),tumospectrum_sensor_parent);
    view_set_previous_callback(number_input_get_view(app->sensor_input),tumospectrum_sensor_previous);
    view_dispatcher_add_view(app->view_dispatcher,TumoSpectrumViewSensorMenu,submenu_get_view(app->sensor_menu));
    view_dispatcher_add_view(app->view_dispatcher,TumoSpectrumViewSensorInput,number_input_get_view(app->sensor_input));
...
static void tumospectrum_free(TumoSpectrumApp* app) {
    ...
    number_input_free(app->sensor_input);submenu_free(app->sensor_menu);furi_string_free(app->sensor_report);

...
static bool
    ...
    if(storage_file_open(file, path, FSAM_WRITE, FSOM_OPEN_APPEND)) {
        ...
        const size_t header_size = header && size == 0 ? strlen(header) : 0;
        const size_t text_size = strlen(text);
        if(size > SPECTER_LOG_MAX_BYTES ||
           header_size + text_size > SPECTER_LOG_MAX_BYTES - size) {
            if(full) *full = true;
        } else {
            ok = true;
            if(header && size == 0) {
            ...
            if(!ok) {
                // Best-effort rollback preserves complete earlier entries. Any
                // failure remains a failure even if rollback itself succeeds.
                if(storage_file_seek(file, (uint32_t)size, true) && storage_file_truncate(file))

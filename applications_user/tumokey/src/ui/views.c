...
static uint32_t zerofido_guide_previous_callback(void *context);
...
static void zerofido_open_startup_reset_confirm(ZerofidoApp *app) {
    zerofido_open_pin_confirm_dialog(app, ZfPinConfirmActionResetAppData, ZfViewStatus,
                                     "Reset TumoKey?", "Clear saved passkeys\nand PIN data",
                                     "Reset");
}
...
static void zerofido_refresh_settings_menu(ZerofidoApp *app) {
    ...
    uint32_t max_index = ZfSettingsItemGuide;
    ...
    submenu_add_item(app->settings_menu, "Guide / About", ZfSettingsItemGuide,
                     zerofido_settings_menu_callback, app);
...
static void zerofido_settings_menu_callback(void *context, uint32_t index) {
    ...
    case ZfSettingsItemStartupReset:
        ...
        break;
    case ZfSettingsItemGuide:
        zerofido_ui_switch_to_view(app, ZfViewGuide);
        break;
    default:
        break;
    }
...
static void zerofido_ui_prune_rare_views(ZerofidoApp *app) {
    ...
    if (!app->approval_view && !app->credential_detail_view && !app->credentials_menu &&
        ...
        !app->guide_view && !app->pin_buffers) {
        return;
    }
    ...
    zerofido_ui_free_rare_view(app, ZfViewGuide);
    ...
    case ZfViewGuide:
        if (!app->guide_view) {
            ...
            widget_add_text_scroll_element(
                ...
                "USB\n"
                "1. Keep USB HID selected.\n"
                "2. Add a passkey in your browser.\n"
                "3. Approve the website on Flipper.\n\n"
                "NFC\n"
                "1. Select NFC in Settings.\n"
                "2. Start passkey setup on iPhone.\n"
                "3. Hold iPhone near Flipper.\n\n"
                "PIN and saved passkeys are managed in Settings and on the home screen.\n\n"
                "Development build. No secure element. Do not use as your only authenticator.\n\n"
                "github.com/squazaryu/tumoflip");
            view_set_previous_callback(
                widget_get_view(app->guide_view), zerofido_guide_previous_callback);
        }
        return zerofido_ui_register_view(
            app, ZfViewGuide, widget_get_view(app->guide_view));

...
static SubGhzRadioBroker* radio_broker;
static SubGhzRadioBrokerLease* radio_lease;

void radio_device_loader_bind(SubGhzRadioBroker* broker, SubGhzRadioBrokerLease* lease) {
    radio_broker = broker;
    radio_lease = lease;
}

static bool radio_device_loader_power_on(void) {
    return radio_broker && radio_lease &&
           subghz_radio_broker_external_power_on(radio_broker, radio_lease);
}

static void radio_device_loader_power_off(void) {
    if(radio_broker && radio_lease)
        subghz_radio_broker_external_power_off(radio_broker, radio_lease);
}

...
static void weather_station_scene_receiver_config_set_frequency(WeatherVariableItem* item) {
    if(!item) return;
    ...
    if(app->txrx->hopper_state == WSHopperStateOFF) {
        ...
        weather_variable_item_set_current_value_text(item, text_buf);
        app->txrx->preset->frequency = subghz_setting_get_frequency(app->setting, index);
    } else {
        weather_variable_item_set_current_value_index(
            item, subghz_setting_get_frequency_default_index(app->setting));
    }
}

static void weather_station_scene_receiver_config_set_preset(WeatherVariableItem* item) {
    if(!item) return;
    WeatherStationApp* app = weather_variable_item_get_context(item);
    if(!app || !app->txrx || !app->txrx->preset || !app->setting) return;
...
static void weather_station_scene_receiver_config_set_hopping_running(WeatherVariableItem* item) {
    if(!item) return;
    WeatherStationApp* app = weather_variable_item_get_context(item);
    if(!app || !app->txrx || !app->txrx->preset || !app->setting || !app->scene_manager) return;
    uint8_t index = weather_variable_item_get_current_value_index(item);
    if(index >= HOPPING_COUNT) index = 0U;
    weather_variable_item_set_current_value_text(item, hopping_text[index]);
...
static void weather_station_scene_receiver_config_build_list(WeatherStationApp* app) {
    ...
    item = weather_variable_item_list_add(
        list,
        "Frequency:",
    ...
    weather_variable_item_set_current_value_index(item, value_index);
    char text_buf[10] = {0};
    if(app->txrx->hopper_state == WSHopperStateOFF) {
        snprintf(
            text_buf,
    ...
    weather_variable_item_set_current_value_index(item, value_index);
    weather_variable_item_set_current_value_text(item, hopping_text[value_index]);
    ...
    weather_variable_item_list_add(list, "Lock controls", 1, NULL, NULL);
    weather_variable_item_list_set_enter_callback(
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == WEATHER_EDITOR_RECEIVER_CONFIG_REFRESH) {
            const uint8_t selected = app->variable_item_list ?
                                         weather_variable_item_list_get_selected_item_index(
                                             app->variable_item_list) :
                                         0U;
            weather_station_scene_receiver_config_build_list(app);
            if(app->variable_item_list) weather_variable_item_list_set_selected_item(

...
WeatherStationApp* weather_station_app_alloc() {
    ...
    // Do not delete files merely because the application was opened.
...
int32_t weather_station_app(void* p) {
    ...
    if(!subghz_radio_broker_acquire(broker, "weather_editor", 0, &lease)) {
        DialogsApp* dialogs = furi_record_open(RECORD_DIALOGS);
        DialogMessage* message = dialog_message_alloc();
        dialog_message_set_text(message, "Sub-GHz radio is busy", 64, 32, AlignCenter, AlignCenter);
    ...
    radio_device_loader_bind(broker, &lease);
    ...
    radio_device_loader_bind(NULL, NULL);

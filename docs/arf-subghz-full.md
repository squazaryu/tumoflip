...
ProtoPirate-derived garage/gate tool. RollJam 2.0 is exposed through the
RollJam entry inside ARF Sub-GHz Full instead of as a second visible ARF Tools
app. The remaining child FAPs are stored in
...
- RollJam 2.0 through the RollJam child entry;
...
Emulate, ProtoPirate, RollJam 2.0, and Bruteforcer stay in separate child FAPs.
The legacy `applications_user/rolljam` source is kept in-tree for reference but
is excluded from reproducible updater packages.

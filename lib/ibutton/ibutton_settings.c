...
iButtonWriteTargetMask ibutton_settings_get_write_targets(void) {
    ...
    if(stat == FSE_OK) {
        ...
        FURI_LOG_W(TAG, "%s unusable, write targets disabled", IBUTTON_SETTINGS_PATH);

    } else if(stat != FSE_NOT_EXIST) {
        FURI_LOG_W(
            TAG,
            "%s unreachable (%s), write targets disabled",
            IBUTTON_SETTINGS_PATH,
            storage_error_get_desc(stat));
    }

    // Only a genuinely missing file is first-run state. Corruption or an unavailable SD
    // must never silently re-enable a blank type the owner may have disabled.
    return stat == FSE_NOT_EXIST ? ibutton_write_targets_default() : 0;
}

...
static bool em4100_can_be_decoded(
    ...
    // Require the next frame's 9-bit header so Electra frames aren't misread as EM4100.
...
bool protocol_em4100_decoder_feed(ProtocolEM4100* proto, bool level, uint32_t duration) {
    ...
    if(event != ManchesterEventReset) {
        ...
        if(data_ok) {
            // Keep the newest 9 bits pending while the preceding 64-bit frame is validated.
            bool carry = (proto->encoded_epilogue >> 8) & 0b1;

            proto->encoded_data = (proto->encoded_data << 1) | carry;
            proto->encoded_epilogue =
                ((proto->encoded_epilogue << 1) | data) & EM_EPILOGUE_HEADER;

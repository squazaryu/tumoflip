...
#define EMV_BCD_DATE_SIZE   3U
#define EMV_BCD_TIME_SIZE   3U
#define EMV_BCD_AMOUNT_SIZE 6U
...
static bool
    ...
    case EMV_TAG_APPL_EFFECTIVE:
        if(tlen != EMV_BCD_DATE_SIZE) return emv_tag_rejected(tag, tlen);
...
// Convert 4-bit to ASCII representation
    ...
    case EMV_TAG_LAST_ONLINE_ATC:
        if(tlen != sizeof(app->last_online_atc)) return emv_tag_rejected(tag, tlen);
        app->last_online_atc = (buff[i] << 8 | buff[i + 1]);
        success = true;
        break;
    case EMV_TAG_ATC:
        if(tlen != sizeof(app->transaction_counter)) return emv_tag_rejected(tag, tlen);
...
static bool
    ...
    if((tlen & 128) == 128) { // long length value
        const uint8_t length_bytes = tlen & 0x7FU;
        if(length_bytes != 1U) {
            FURI_LOG_W(TAG, "Unsupported TLV length header 0x%02X", tlen);
            return false;
        }
        i++;
        if(i >= len) return success;

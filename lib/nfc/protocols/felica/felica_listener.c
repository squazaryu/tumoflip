...
static FelicaError felica_listener_command_handler_request_system_code(
    ...
    uint32_t system_count = simple_array_get_count(instance->data->systems);
    const uint32_t system_count_max =
        (FELICA_LISTENER_MAX_BUFFER_SIZE - 2U - sizeof(FelicaCommandHeaderRaw) - 1U) /
        sizeof(uint16_t);
    if(system_count > system_count_max) system_count = system_count_max;
...
NfcCommand felica_listener_run(NfcGenericEvent event, void* context) {
    ...
    if(nfc_event->type == NfcEventTypeFieldOn) {
        ...
        do {
            const size_t frame_size = bit_buffer_get_size_bytes(nfc_event->data.buffer);
            if(frame_size < 2U + FELICA_CRC_SIZE) {
                FURI_LOG_E(TAG, "Request is too short");
                break;
            }
            ...
            const size_t size = frame_size - FELICA_CRC_SIZE;
            if(!felica_listener_validate_request(instance, request, size)) {

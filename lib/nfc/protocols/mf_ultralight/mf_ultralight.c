...
#define MF_ULTRALIGHT_AES_DATA_KEY         "AES Data Key"
...
MfUltralightData* mf_ultralight_alloc(void) {
    MfUltralightData* data = calloc(1, sizeof(MfUltralightData));
    furi_check(data);
...
void mf_ultralight_reset(MfUltralightData* data) {
    ...
    data->aes_data_key_present = false;
    mbedtls_platform_zeroize(&data->aes_data_key, sizeof(data->aes_data_key));
}
...
void mf_ultralight_copy(MfUltralightData* data, const MfUltralightData* other) {
    ...
    data->aes_data_key_present = other->aes_data_key_present;
    data->aes_data_key = other->aes_data_key;
...
bool mf_ultralight_load(MfUltralightData* data, FlipperFormat* ff, uint32_t version) {
    ...
    do {
        ...
        // Explicit optional metadata. Legacy dumps do not contain it and remain loadable. Never
        // infer presence from an all-zero key: zero is a valid AES key and key pages read as zero.
        data->aes_data_key_present = flipper_format_read_hex(
            ff,
            MF_ULTRALIGHT_AES_DATA_KEY,
            data->aes_data_key.data,
            sizeof(data->aes_data_key.data));
        if(!data->aes_data_key_present) {
            mbedtls_platform_zeroize(&data->aes_data_key, sizeof(data->aes_data_key));
        }
...
bool mf_ultralight_is_equal(const MfUltralightData* data, const MfUltralightData* other) {
    ...
    do {
        ...
        if(memcmp(data->aes_signature, other->aes_signature, sizeof(data->aes_signature)) != 0)
            break;
        if(data->aes_data_key_present != other->aes_data_key_present) break;
        if(data->aes_data_key_present &&
           memcmp(&data->aes_data_key, &other->aes_data_key, sizeof(data->aes_data_key)) != 0)
            break;

...
static void mf_ultralight_listener_send_response(MfUltralightListener* instance) {
    if(instance->aes_sec_msg) {
        ...
        if(!mf_ultralight_aes_cmac8_ctr(
            ...
            mbedtls_platform_zeroize(mac8, sizeof(mac8));
...
static bool mf_ultralight_aes_verify_strip_cmd(MfUltralightListener* instance, BitBuffer* rx) {
    ...
    uint8_t mac8[MF_ULTRALIGHT_AES_CMAC_SIZE] = {0};
    const bool valid = mf_ultralight_aes_cmac8_ctr(
                           instance->aes_cmac_key,
                           instance->aes_cmac_ctr,
                           data,
                           cmd_len,
                           mac8) &&
                       mf_ultralight_aes_equal(
                           mac8, data + cmd_len, MF_ULTRALIGHT_AES_CMAC_SIZE);
    mbedtls_platform_zeroize(mac8, sizeof(mac8));
...
static bool mf_ultralight_aes_listener_crypt(
    const uint8_t* key,
    int mode,
    size_t length,
    const uint8_t* input,
    ...
    int error = (mode == MBEDTLS_AES_ENCRYPT) ?
                    mbedtls_aes_setkey_enc(&ctx, key, 128) :
                    mbedtls_aes_setkey_dec(&ctx, key, 128);
    if(error == 0) error = mbedtls_aes_crypt_cbc(&ctx, mode, length, iv, input, output);
    mbedtls_platform_zeroize(iv, sizeof(iv));
    mbedtls_aes_free(&ctx);
    if(error != 0) mbedtls_platform_zeroize(output, length);
    return error == 0;
}
...
static MfUltralightCommand
    ...
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    uint8_t key[MF_ULTRALIGHT_AES_KEY_SIZE] = {0};
    uint8_t decrypted[2 * MF_ULTRALIGHT_AES_BLOCK_SIZE] = {0};
    uint8_t expected[MF_ULTRALIGHT_AES_BLOCK_SIZE] = {0};
    uint8_t rnd_a_rot[MF_ULTRALIGHT_AES_BLOCK_SIZE] = {0};
    uint8_t rnd_b_rot[MF_ULTRALIGHT_AES_BLOCK_SIZE] = {0};
    uint8_t encrypted_rnd_a[MF_ULTRALIGHT_AES_BLOCK_SIZE] = {0};
    ...
    do {
        ...
        if(!mf_ultralight_aes_get_key(instance->data, key)) break;

        // Decrypt ek(RndA || RndB') -> RndA || RndB'
        const uint8_t* enc = bit_buffer_get_data(buffer) + 1;
        if(!mf_ultralight_aes_listener_crypt(
               key,
               MBEDTLS_AES_DECRYPT,
               sizeof(decrypted),
               enc,
        ...
        uint8_t* rnd_a = decrypted;
        const uint8_t* rnd_b_prime = decrypted + MF_ULTRALIGHT_AES_BLOCK_SIZE;
        ...
        if(mf_ultralight_aes_equal(rnd_b_prime, expected, sizeof(expected))) {
            instance->auth_state = MfUltralightListenerAuthStateSuccess;
            ...
            if(instance->data->page[MF_ULTRALIGHT_AES_CFG_PAGE].data[0] &
               MF_ULTRALIGHT_AES_CFG_SEC_MSG_ACT) {
                memcpy(rnd_a_rot, rnd_a, sizeof(rnd_a_rot));
                memcpy(rnd_b_rot, instance->aes_rnd_b, sizeof(rnd_b_rot));
                mf_ultralight_aes_rol16(rnd_a_rot);
                mf_ultralight_aes_rol16(rnd_b_rot);
                if(!mf_ultralight_aes_derive_session_key(
                       key, rnd_a_rot, rnd_b_rot, instance->aes_cmac_key)) {
        ...
        bit_buffer_append_bytes(
            instance->tx_buffer, encrypted_rnd_a, sizeof(encrypted_rnd_a));
        iso14443_3a_listener_send_standard_frame(
            instance->iso14443_3a_listener, instance->tx_buffer);
        command = MfUltralightCommandProcessed;
    } while(false);
    ...
    mbedtls_platform_zeroize(rnd_a_rot, sizeof(rnd_a_rot));
    mbedtls_platform_zeroize(rnd_b_rot, sizeof(rnd_b_rot));
    mbedtls_platform_zeroize(encrypted_rnd_a, sizeof(encrypted_rnd_a));
    mbedtls_platform_zeroize(instance->aes_rnd_b, sizeof(instance->aes_rnd_b));
    return command;
}
...
static MfUltralightCommand
    ...
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    uint8_t key[MF_ULTRALIGHT_AES_KEY_SIZE] = {0};
    uint8_t encrypted_rnd_b[MF_ULTRALIGHT_AES_BLOCK_SIZE] = {0};
    ...
    do {
        ...
        instance->aes_sec_msg = false;
        instance->aes_cmac_ctr = 0;
        mbedtls_platform_zeroize(instance->aes_cmac_key, sizeof(instance->aes_cmac_key));

        // The dump contains only DataProtKey. Reject UIDRetrKey and OriginalityKey selectors
        // instead of authenticating those independent key domains with the wrong stored key.
        if(bit_buffer_get_byte(buffer, 1) != MfUltralightAesKeyTypeData) break;
        if(!mf_ultralight_aes_get_key(instance->data, key)) break;
        ...
        bit_buffer_append_byte(instance->tx_buffer, 0xAF);
        bit_buffer_append_bytes(
            instance->tx_buffer, encrypted_rnd_b, sizeof(encrypted_rnd_b));
        iso14443_3a_listener_send_standard_frame(
            instance->iso14443_3a_listener, instance->tx_buffer);
        command = MfUltralightCommandProcessed;
        mf_ultralight_composite_command_set_next(
    ...
    mbedtls_platform_zeroize(key, sizeof(key));
    mbedtls_platform_zeroize(encrypted_rnd_b, sizeof(encrypted_rnd_b));
    return command;
}
...
MfUltralightListener* mf_ultralight_listener_alloc(
    ...
    MfUltralightListener* instance = calloc(1, sizeof(MfUltralightListener));
    furi_check(instance);

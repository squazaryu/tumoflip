...
MfUltralightError mf_ultralight_poller_authenticate_aes(
    ...
    do {
        ...
        if(mbedtls_aes_setkey_enc(&ctx, key, 128) != 0 ||
           mbedtls_aes_crypt_cbc(
               &ctx, MBEDTLS_AES_ENCRYPT, sizeof(rnd_ab), iv, rnd_ab, enc_ab) != 0) {
            ret = MfUltralightErrorProtocol;
            break;
        }
        ...
        if(!mf_ultralight_aes_equal(rec_rnd_a, rnd_a, MF_ULTRALIGHT_AES_BLOCK_SIZE)) {
            ret = MfUltralightErrorAuth;
            break;
        }
        ...
        if(!mf_ultralight_aes_derive_session_key(
               key, rnd_a, rnd_b, instance->aes_cmac.session_key)) {
            ret = MfUltralightErrorProtocol;
            break;
        }
    ...
    mbedtls_platform_zeroize(rnd_ab, sizeof(rnd_ab));
    mbedtls_platform_zeroize(enc_ab, sizeof(enc_ab));
    mbedtls_platform_zeroize(cmd2, sizeof(cmd2));
    mbedtls_platform_zeroize(rec_rnd_a, sizeof(rec_rnd_a));
...
static MfUltralightError mf_ultralight_poller_txrx_aes_cmac(
    ...
    uint8_t rmac8[MF_ULTRALIGHT_AES_CMAC_SIZE] = {0};
    if(!mf_ultralight_aes_cmac8_ctr(
           instance->aes_cmac.session_key, ctr, cmd, cmd_len, mac8)) {
    ...
    do {
        ...
        if(!mf_ultralight_aes_cmac8_ctr(
               instance->aes_cmac.session_key, ctr + 1, rx, resp_len, rmac8) ||
           !mf_ultralight_aes_equal(
               rmac8, rx + resp_len, MF_ULTRALIGHT_AES_CMAC_SIZE)) {
    ...
    mbedtls_platform_zeroize(rmac8, sizeof(rmac8));
    return ret;
}

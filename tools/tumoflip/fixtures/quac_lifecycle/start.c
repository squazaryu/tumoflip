...
#define REQUIRE(x)                                                  \
    do {                                                            \
        if(!(x)) {                                                  \
            fprintf(stderr, "TX start invariant failed: %s\n", #x); \
            return 1;                                               \
        }                                                           \
    } while(0)
typedef struct {
...
static bool flipper_format_rewind(FlipperFormat* f) {
    return !is("rewind");
}
static bool flipper_format_read_string(FlipperFormat* f, const char* k, FuriString* s) {
    s->text = "Princeton";
    return !is("protocol");
}
static bool
    flipper_format_insert_or_update_uint32(FlipperFormat* f, const char* k, uint32_t* v, size_t n) {
    return !is("repeat");
}
static void subghz_txrx_stop(SubGhzTxRx* t) {
    if(t->txrx_state == SubGhzTxRxStateTx) saves++;
}
static void* subghz_transmitter_alloc_init(void* env, const char* protocol) {
    return is("no-encoder") ? NULL : malloc(1);
}
static SubGhzProtocolStatus subghz_transmitter_deserialize(void* t, FlipperFormat* f) {
    return is("deserialize") ? SubGhzProtocolStatusError : SubGhzProtocolStatusOk;
}
static void* subghz_setting_get_preset_data_by_name(void* s, const char* n) {
    return s;
}
static void subghz_txrx_begin(SubGhzTxRx* t, void* data) {
    t->txrx_state = SubGhzTxRxStateIDLE;
}
static bool subghz_txrx_tx(SubGhzTxRx* t, uint32_t f) {
    if(is("only-rx")) return false;
    t->txrx_state = SubGhzTxRxStateTx;
    return true;
}
static void subghz_transmitter_yield(void) {
}
static bool subghz_devices_start_async_tx(void* device, void (*cb)(void), void* ctx) {
    async_calls++;
    return !is("async-failure");
}
static void subghz_txrx_radio_state(SubGhzTxRx* t, int state) {
    broker_async++;
}
static void subghz_transmitter_free(void* t) {
    if(t) {
        frees++;
        free(t);
    }
}
static void subghz_txrx_idle(SubGhzTxRx* t) {
    t->txrx_state = SubGhzTxRxStateIDLE;
}
...
int main(int argc, char** argv) {
    ...
    REQUIRE(
        frees == (is("no-encoder") || is("rewind") || is("protocol") || is("repeat") ? 0U : 1U));
    return 0;
}

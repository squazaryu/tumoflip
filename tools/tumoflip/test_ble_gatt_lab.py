...
class BleGattLabTest(unittest.TestCase):
    ...
    def test_log_and_reconnect_safety(self) -> None:
        ...
        self.assertIn("furi_hal_bt_gatt_connect", self.source)
        self.assertIn("furi_hal_bt_gatt_disconnect", self.source)
        self.assertIn("furi_hal_bt_gatt_discover_services", self.source)
        self.assertIn("furi_hal_bt_gatt_discover_characteristics", self.source)
        self.assertIn("furi_hal_bt_gatt_read", self.source)
        self.assertIn("furi_hal_bt_gatt_write", self.source)
        self.assertIn("furi_hal_bt_gatt_set_notifications", self.source)
        self.assertIn('strcmp(event->command, "notify") == 0', self.source)
        self.assertIn("CCCD handle", self.source)
        self.assertIn("explicit confirm required", self.source)
        self.assertIn("Full BLE stack required", self.source)
        self.assertIn("bool authorized;", self.source)
        self.assertIn("BleGattLabPendingOperation", self.source)
        self.assertIn("Confirm GATT write", self.source)
        self.assertIn("Confirm notification change", self.source)
        self.assertIn("user_cancelled", self.source)
        self.assertIn("operation_pending", self.source)
        self.assertIn("<redacted>;len=", self.source)
        self.assertIn("BLE_GATT_LAB_EVENT_DATA_PREVIEW_MAX", self.source)
        self.assertIn("Use only with permission", self.source)
        self.assertIn('"authorization_required"', self.source)
        self.assertIn("if(!app->authorized)", self.source)
    ...
    def test_cockpit_package_and_docs_route_lab(self) -> None:
        ...
        self.assertIn("scan_start", self.bridge_doc)
        self.assertIn("characteristics", self.bridge_doc)
        self.assertIn("notify", self.bridge_doc)
        self.assertIn("unconfirmed or oversized writes are rejected", self.bridge_doc)

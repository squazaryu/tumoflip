...
#define FURI_LOG_E(tag,format,operation,status) ((void)(operation))
...
int hci_le_set_address_resolution_enable(uint8_t enabled){
 if(ll_only_hci_unsupported){unsupported_calls++;return 0x01;}
 if(enabled)assert(stage==1);
 stage=2;return failure==4;
}
static int aci_gap_configure_filter_accept_list(void){assert(stage==0);stage=3;return failure==5;}
...
int main(void){
 ...
 for(failure=0;failure<=4;failure++){stage=0;assert(gap_peer_select(&p)==(failure!=3));}
 ...
 p.address_type=0;failure=0;stage=0;ll_only_hci_unsupported=true;
 assert(gap_peer_select(&p));
 stage=0;assert(gap_peer_select(NULL));
 assert(unsupported_calls==0);
 assert(!gap_peer_read(NULL));return 0;
}
''')

    def test_select_and_pair_with_only_host_stack_commands(self):
        source = (ROOT / "targets/f7/ble_glue/gap_peer_policy.c").read_text()
        body = ""
        if "static bool gap_peer_command_ok(" in source:
            body += function(source, "static bool gap_peer_command_ok(") + "\n"
        body += function(source, "bool gap_peer_select(")
        run_c(r'''
#include <assert.h>
#include <stdbool.h>
#include <stdint.h>
...
typedef struct {uint8_t Address_Type,Address[6];} List_Entry_t;
static int selected,pairing,raw_hci;
int hci_le_set_address_resolution_enable(uint8_t enabled){
 (void)enabled;raw_hci++;return 0x01; // Unknown HCI Command on host+controller stack
}
static int aci_gap_is_device_bonded(uint8_t type,const uint8_t* address){
 assert(type==0 && address[0]==1);return 0;
}
static int aci_gap_add_devices_to_list(uint8_t count,const List_Entry_t* entry,uint8_t mode){
 assert(count==1 && mode==5 && entry->Address_Type==0 && entry->Address[0]==1);
 selected++;return 0;
}
static int aci_gap_configure_filter_accept_list(void){pairing++;return 0;}
''' + body + r'''
int main(void){
 GapBondedDevice peer={0,{1,1,1,1,1,1}};
 assert(gap_peer_select(&peer));assert(selected==1 && pairing==0 && raw_hci==0);
 assert(gap_peer_select(NULL));assert(selected==1 && pairing==1 && raw_hci==0);
 return 0;
}
''')

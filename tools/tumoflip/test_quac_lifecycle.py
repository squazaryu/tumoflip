...
def llvm_tool(name):
    return (
        shutil.which(name)
        or subprocess.check_output(["xcrun", "--find", name], text=True).strip()
    )
...
class QuacLifecycleTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        targets = {
            "ir": (
                ...
                (
                    "infrared_utils_signal_alloc",
                    "infrared_utils_signal_free",
                    "infrared_utils_read_signal_at_index",
                    "infrared_utils_write_signal",
                ),
            ),
            "subghz": (
                "actions/action_subghz.c",
                (
                    "action_subghz_raw_end_callback",
                    "action_subghz_tx",
                ),
            ),
        ...
        for target, (relative, names) in targets.items():
            ...
            combined = template.replace(
                "/* @PRODUCTION@ */", f'#include "{production}"'
            )
            ...
            with self.subTest(target=target, case=case):
                ...
                if COVERAGE:
                    environment["LLVM_PROFILE_FILE"] = str(
                        self.directory / f"{target}-%p.profraw"
                    )
                result = subprocess.run(
                    [str(self.binaries[target]), case],
                    env=environment,
                    capture_output=True,
                    text=True,
                    timeout=5,
    ...
    def test_ir_reused_import_signal(self):
        self.run_cases(
            "ir", ("raw-raw", "raw-parsed", "parsed-raw-failure", "raw-failure")
        )
    ...
    def test_backend_honors_async_start_failure(self):
        self.run_cases(
            "start", ("async-failure", "only-rx", "deserialize", "no-encoder")
        )
    ...
    def test_safety_patch_identity_and_ci_gates(self):
        self.assertIn('fap_version="0.10.0"', (QUAC / "application.fam").read_text())
        for workflow in ("pr-build.yml", "release.yml"):
            text = (ROOT / ".github/workflows" / workflow).read_text()
            self.assertTrue(
                "tools/tumoflip/test_quac_lifecycle.py" in text,
